jQuery(function($) {
	try {
		...
		$('#cm_action').empty().html('&nbsp;Create Content Group');
		$('#cm_action').click(function() {
...
function updateContentGroupEnabled(pContentGroupId, pContentGroupEnabled, pElementName) {
	openWait();
				...
				.ajax({
					...
					statusCode : {
						200 : function(contentgroup) {
							...
							$('#contentgroup_description')
									.val(contentgroup.description);
							$('#contentgroup_start_date').val(
									getDisplayDate(contentgroup.startDateIso8601));
							$('#contentgroup_end_date').val(
									getDisplayDate(contentgroup.endDateIso8601));
							// add more
							$('#contentgroup_userid').val(contentgroup.sponsoredUserId);
							...
							$('#contentgroup_cancel_button').one('click',
									function() {
										$('#contentgroup_save_button').unbind();
										;
									});
...
function createContentGroup() {
	...
	try {
		...
		var contentgroupObj = {
			...
			startDateIso8601 : getTransferDate($('#contentgroup_start_date').val()),
		...
		var jqxhr = $.ajax({
			...
			statusCode : {
				201 : function() {
					$('#content_group_create_modal').foundation('reveal', 'close');
					getContentGroups();
				},
				400 : function(text) {
					try {
...
function deleteContentGroup(id) {
	...
	try {
		...
		displayConfirm("Are you sure you want to delete this ContentGroup?",
				function() {
					wait("confirm_wait");
					var url = "secured/contentgroup/" + id + "/" + _timeUpdatedMs
