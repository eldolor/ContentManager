...
function displayConfirm(message, callback) {
	...
	/*
	 * $('').modal({ closeHTML : "<a href='#' title='Close'
	 * class='modal-close'>x</a>", position : [ "20%", ], overlayId :
	 * 'confirm-overlay', containerId : 'confirm-container', onShow :
	 * function(dialog) { var modal = this;
	 * 
	 * $('.message', dialog.data[0]).append(message); // if the user clicks
	 * "yes" $('.yes', dialog.data[0]).click(function() { // call the callback
	 * if ($.isFunction(callback)) { callback.apply(); } // close the dialog
	 * modal.close(); // or ; }); } });
	 */
}
