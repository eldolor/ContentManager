...
(function(undefined) {
	...
	function loadLocale(name) {
		...
		if (!locales[name] && hasModule) {
			try {
				...
				// we want to undo that for lazy loaded locales
	...
	function relativeTime(posNegDuration, withoutSuffix, locale) {
		var duration = moment.duration(posNegDuration).abs(), seconds = round(duration
				...
				.as('M')), years = round(duration.as('y')),
	...
	moment.lang = deprecate(
			'moment.lang is deprecated. Use moment.locale instead.', function(
					key, value) {
	...
	extend(
			moment.fn = Moment.prototype,
			{
				...
				calendar : function(time) {
					...
					// not.
					var now = time || moment(), sod = makeAs(now, this)
							.startOf('day'), diff = this
									...
									: diff < 1 ? 'sameDay'
											: diff < 2 ? 'nextDay'
													: diff < 7 ? 'nextWeek'
															: 'sameElse';
					return this
							.format(this.localeData().calendar(format, this));
				},
				...
				endOf : function(units) {
					...
					return this.startOf(units).add(1,
							(units === 'isoWeek' ? 'week' : units)).subtract(1,
							'ms');
				},
				...
				// true)]-->
				// 5:31:26 +0200 It is possible that 5:31:26 doesn't exist int
				// zone
	...
	extend(
			moment.duration.fn = Duration.prototype,
			{
				...
				humanize : function(withSuffix) {
					var output = relativeTime(this, !withSuffix, this
							.localeData());
